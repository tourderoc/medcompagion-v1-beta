...
namespace MedCompanion.Services.Restitutions
{
    ...
    public class RestitutionSuggesterService
    {
        ...
        private static string BuildPtS3Instruction(DossierReading reading)
        {
            ...
            sb.AppendLine("  • echeance : « sans attendre » | « sous 1 mois » | « ce trimestre » | « cette année scolaire » | « à 6 mois » | « à 1 an » | « à 2 ans »");
            sb.AppendLine("  • degre : « indispensable » | « recommandé » | « utile si possible » | « à réévaluer plus tard »");
            sb.AppendLine("  • indication.degre : les mêmes, PLUS « non indiqué à ce stade » — qui ne vaut que pour");
            sb.AppendLine("      la section entière, jamais pour une action isolée.");
            sb.AppendLine();
            sb.AppendLine("L'INDICATION COMMANDE TOUTE LA SECTION — c'est la première chose à trancher :");
            sb.AppendLine("  Un soutien développemental n'est pas automatique. Un enfant peut n'avoir aucun besoin");
            sb.AppendLine("  particulier à ce niveau, et c'est une information clinique, pas un manque. Deux cas :");
            sb.AppendLine();
            sb.AppendLine("  (A) INDIQUÉ → remplis intro, objectifs, reeducations, ressourcesVie, reperesEvolution,");
            sb.AppendLine("      reevaluation. Laisse « critereReevaluation » vide \"\".");
            sb.AppendLine();
            sb.AppendLine("  (B) NON INDIQUÉ ou DIFFÉRÉ → renseigne « motif » et « critereReevaluation », et laisse");
            sb.AppendLine("      intro, objectifs, reeducations, ressourcesVie, reperesEvolution, reevaluation VIDES.");
            sb.AppendLine("      Dire « le développement est harmonieux, rien à engager à ce stade, à reconsidérer si");
            sb.AppendLine("      tel élément apparaît » est une information utile. Aligner des rééducations sous une");
            sb.AppendLine("      indication qu'on vient d'écarter serait se contredire dans la même page.");
            sb.AppendLine();
            sb.AppendLine("  • motif : ancré dans CE dossier — les éléments qui fondent (ou non) l'indication.");
            sb.AppendLine("    Jamais de généralité.");
            sb.AppendLine();
        ...
        private static string BuildPtS4Instruction(DossierReading reading)
        {
            ...
            sb.AppendLine("  \"indication\": {");
            sb.AppendLine("    \"degre\": \"...\", \"porteur\": \"...\",");
            sb.AppendLine("    \"motif\": \"Pourquoi un accompagnement familial EST ou N'EST PAS indiqué pour CETTE famille.\",");
            sb.AppendLine("    \"critereReevaluation\": \"Ce qui ferait reconsidérer — uniquement s'il est différé ou non indiqué.\"");
            sb.AppendLine("  },");
            ...
            sb.AppendLine();
            sb.AppendLine("L'INDICATION COMMANDE TOUTE LA SECTION — c'est la première chose à trancher :");
            sb.AppendLine("  Certaines familles n'ont besoin d'aucun accompagnement : des parents qui tiennent leur");
            sb.AppendLine("  rôle, un cadre cohérent, une demande claire. Le dire est une information clinique, pas");
            sb.AppendLine("  un manque — et l'écrire évite de proposer une guidance à qui n'en a pas besoin. Deux cas :");
            sb.AppendLine();
            sb.AppendLine("  (A) INDIQUÉ → remplis intro, objectifs, accompagnementParents, interventionsEducatives,");
            sb.AppendLine("      auQuotidien, reperesEvolution. Laisse « critereReevaluation » vide \"\".");
            sb.AppendLine();
            sb.AppendLine("  (B) NON INDIQUÉ ou DIFFÉRÉ → renseigne « motif » et « critereReevaluation », et laisse");
            sb.AppendLine("      intro, objectifs, accompagnementParents, interventionsEducatives, auQuotidien,");
            sb.AppendLine("      reperesEvolution VIDES. Aligner des propositions sous une indication qu'on vient");
            sb.AppendLine("      d'écarter serait se contredire dans la même page.");
            sb.AppendLine();
            sb.AppendLine("  • motif : ancré dans CE dossier — ce qui fonde (ou non) l'indication. Jamais de généralité.");
            sb.AppendLine();
