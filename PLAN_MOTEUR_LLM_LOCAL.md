...
- **Bonsai 2 27B : essayé et écarté le 26/09/2026. Ne pas réessayer sur ce matériel.** C'est Qwen3.8-27B compressé en ternaire par PrismML (5,9 à 7,2 Go au lieu de 12,6), sous Apache 2.0. Installé, mesuré, puis retiré — fichiers et moteur supprimés. Les raisons, dans l'ordre :
  1. **Le vocabulaire clinique français est faux.** Sur quatre essais : « Q-AAD-7 », « AESE », « CONARS/VAD », « TAD », « TDI », « AEH », « TPE », « Conducte ». Qwen, même question : WISC-V, Conners-3, Vanderbilt, CMPP, PPS/PAI. C'est ce que coûte la compression sur des termes rares — exactement ceux des courriers et des restitutions. Rédhibitoire.
  2. **Plus lent là où ça compte** : 22 à 27 s pour répondre contre 14 s à Qwen. Non par manque de débit (47 t/s contre 41,8) mais parce qu'il produit deux à trois fois plus de tokens — 409 à 557 de réflexion contre 159. Ni l'effort « low », ni un plafond de réflexion à 800 n'y changent plus de cinq secondes.
  3. **Pas de MTP possible** : la compression a supprimé les couches, le serveur refuse de démarrer (`model doesn't contain MTP layers`). Aucun brouillon externe non plus — la famille Qwen3.8 n'a pas de petit modèle (le 27B, un 2,4 T, et Flash-Next à 180 milliards d'une autre architecture).
  4. **Le matériel ne le sauvera pas.** Mesuré à `llama-bench -ngl 99 -fa 1` : 47,3 t/s ici, contre 54,4 sur RTX 3090, 81-91 sur 4090, 130-142 sur 5090. Le noyau ternaire est limité par le CALCUL, pas par la bande passante — la 3090 a deux fois notre bande passante pour 15 % de gain. Même la 3090 envisagée pour 2027 le laisserait plus lent que Qwen.
  5. Le format PTQ1_0 (5,5 Go) est **plus lent** que PQ2_0 (6,7 Go) ici : 44,5 contre 47,3 t/s. Il ne gagne que sur les cartes limitées par la bande passante.

  Ce qu'il apportait — 24,2 Go de modèles au lieu de 30,5, donc un cache tenable — s'obtient mieux en ajoutant de la RAM. Exige le fork `PrismML-Eng/llama.cpp` (branche prism) : le llama.cpp standard refuse ces fichiers ou produit du charabia. Ce fork, vérifié, fait tourner Qwen + MTP, Gemma + MTP et Qwen3-VL à l'identique — utile à savoir s'il faut y revenir.
...
| 26/09/2026 | — | **Bonsai 2 27B essayé et écarté** — mesures et raisons en section 4. Fichiers et moteur PrismML supprimés (12,4 Go libérés), profil retiré, `LlamaCppExePath` rétabli. |
| 26/09/2026 | 5 | **Le contexte de Gemma tournait encore à 131072** alors que la mesure du 18/09 avait fixé 32768. Le garde-fou prévu pour ça (« chaîne à quatre champs = d'avant le 18/09, on ne relit pas le contexte ») a été **défait par une sauvegarde ultérieure** : la chaîne est repassée à cinq champs en gardant l'ancien 131072, donc relue comme légitime. Réglage corrigé à la main dans `appsettings.json`. **Leçon : un garde-fou qui repose sur la FORME d'une valeur persistée ne survit pas à la première réécriture.** Pour une valeur par défaut qui change, il faut un numéro de version des réglages. |
| 26/09/2026 | 7 | **Essai demandé par le médecin : marge de la pré-lecture abaissée de 2 Go à 512 Mo** (donc 1 Go exigé avant de commencer au lieu de 4). Le journal indique désormais la RAM disponible avant → après chaque lecture. Ce qu'on cherche : est-ce que le cache tient ? Constat d'avant l'essai — les pré-lectures de Qwen aboutissent (12 916 Mo) mais **toujours à 380-470 Mo/s, jamais « déjà en cache »** : il est évincé entre deux passages. 139 lignes « ignoré » ou « interrompu » sur 781. Hypothèse : 32 Go ne suffisent pas pour 29 Go de modèles + 11 Go de travail. |
| 26/09/2026 | — | **RAM : achat recommandé.** AB350 Pro4, 4 emplacements, **maximum 64 Go** ; installé 2 × 16 Go à 2667 MT/s, deux emplacements libres. Cible 4 × 16 Go, de préférence un kit de quatre barrettes identiques (mélanger deux kits sur B350 = plantages aléatoires). Gain attendu : bascule vers Qwen de ~38 s à ~5 s. Aucun gain de débit à espérer (on restera à 2667), c'est de la place de cache qu'on achète. |
